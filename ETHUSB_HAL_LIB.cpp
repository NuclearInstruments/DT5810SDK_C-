...
namespace NIHAL
{
	...
	NI_RESULT NI_HAL::WriteArray(UINT32 *value, UINT32 address, UINT32 length)
	{
		...
		else
		{
			if (Connected == 2)
			{
				if (niUSB->WriteToFPGA(value, address, length) == NI_OK)
					return niUSB->WriteReg(value[0], address);						//ACK PERCHE NON SCRIVE IL PRIMO REGISTRO
			}
			else
			{
				return NI_ERROR_INTERFACE;
