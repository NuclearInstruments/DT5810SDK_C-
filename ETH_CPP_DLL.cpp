...
namespace NIETHPHY
{
	...
	NI_RESULT NI_ETHPHY::ConnectToHost(int PortNo, char* IPAddress)
	{
		...
		target.sin_port = htons((u_short)PortNo); //Port to connect on
		...
		// Set a timeout of 5 seconds for recv() from this socket
		...
		{
			//sprintf(err_log_txt, "connect failed: %d\n", WSAGetLastError());
			//WriteLogFile(err_log_txt);
		...
		{
			//sprintf(err_log_txt, "connect ok: \n");
			//WriteLogFile(err_log_txt);
